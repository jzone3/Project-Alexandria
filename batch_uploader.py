...
# 	username = choice(["emanresu", "shyguy", "shaneybo","saucey","pikachun","lartple","coldshoulder","distargirl","jarson5","weakev","jonhar", "oxacuk", "ollypop", "zfinter", "korile1", "sinkra", "jojo", "bert95", "mickey", "ghost_man", 'vodkacereal', 'jadefirefly', 'sransbury', 'razminr11', 'warsaurus', 'arufe'])
	
# 	guide = guides[i]
# 	title, subject, teacher, tags, file_url = guide
...
# 	ff = files.blobstore.create(mime_type=headers['content-type'], _blobinfo_uploaded_filename=filename)
# 	with files.open(ff, 'a') as data:
# 		data.write(result.content)
# 	files.finalize(ff)
# 	blob_key = files.blobstore.get_blob_key(ff)
